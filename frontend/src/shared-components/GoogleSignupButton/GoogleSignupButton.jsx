...
const GoogleSignupButton = ({ type }) => {
  ...
  useEffect(() => {
    ...
    if (isLoginError) {
      toast.error(loginError?.response?.data?.message ?? 'Something went wrong')
    }
  }, [isLoginError, isLoginSuccess, loginError?.response?.data?.message, navigate])
  ...
  }, [isError, isSuccess, error?.response?.data?.message, navigate])
